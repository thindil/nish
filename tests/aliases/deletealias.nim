...
assert deleteAlias(initLimitedString(capacity = 8, text = "delete 2"), historyIndex, myaliases, db) == QuitSuccess
assert parseInt(db.getValue(sql"SELECT COUNT(*) FROM aliases")) == 1
assert deleteAlias(initLimitedString(capacity = 9, text = "delete 22"), historyIndex, myaliases, db) == QuitFailure
